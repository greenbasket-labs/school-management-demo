...
async function main() {
  ...
  if (existingSchools.length > 0) {
    throw new Error(
      "Demo seed expects a dedicated empty database. Refusing to modify an existing school database.",
    );
  }
  ...
  if (!ownerRole) {
    throw new Error(
      "RBAC is not initialized. Run the normal RBAC seed before the demo seed.",
    );
  }
  ...
  const school = await db.orm.public.School.create({
    ...
    registrationInfo:
      "Demo institution for Green Basket product demonstration",
  });
  ...
  for (const schoolClass of classes) {
    for (let i = 0; i < subjectRecords.length; i++) {
      ...
      await db.orm.public.ClassSubject.create({
        ...
        teacherId:
          teacherRecords[
            (i + classes.indexOf(schoolClass)) % teacherRecords.length
          ].id,
      });
    }
  }
  ...
  for (let i = 0; i < 192; i++) {
    ...
    const lastName =
      lastNames[(i * 3 + Math.floor(i / firstNames.length)) % lastNames.length];

    const studentPortal = i < 12 || i % 24 === 0;
    ...
    const parentFirstName =
      parentFirstNames[(i * 5) % parentFirstNames.length];

    const parentLastName =
      parentLastNames[
        (i * 7 + Math.floor(i / parentLastNames.length)) %
          parentLastNames.length
      ];
    ...
    for (const student of students) {
      ...
      for (let f = 0; f < feeTypes.length; f++) {
        await db.orm.public.FeeAssignment.create({
          ...
          description: `${feeTypes[f].name} â€” ${term.name}`,
          status: "ACTIVE",
        });
      }
    }
  ...
  for (const student of students) {
    const assignments = allFeeAssignments.filter(
      (a) => a.studentId === student.id,
    );

    for (let termIndex = 0; termIndex < terms.length; termIndex++) {
      const termAssignments = assignments.filter(
        (a) => a.termId === terms[termIndex].id,
      );

      const total = termAssignments.reduce(
      ...
      if (student.index % 12 === 0) {
        paymentAmount = total;
      } else if (student.index % 5 === 0) {
        paymentAmount = 125000;
      } else if (student.index % 3 === 0) {
      ...
      paymentDate.setUTCDate(
        paymentDate.getUTCDate() + 12 + (student.index % 12),
      );
  ...
  for (let termIndex = 0; termIndex < terms.length; termIndex++) {
    for (let classIndex = 0; classIndex < classes.length; classIndex++) {
      ...
      for (
        let subjectIndex = 0;
        subjectIndex < subjectRecords.length;
        subjectIndex++
      ) {
        ...
        const examMark =
          await db.orm.public.AssessmentComponent.create({
            examSubjectId: examSubject.id,
            name: "Examination",
            maxMark: 70,
            sortOrder: 2,
        ...
        const classStudents = students.filter(
          (student) => student.classId === classRecord.id,
        );
        ...
        const orderedScores = classStudents.map((student) => {
          ...
          return {
            student,
            total: Math.min(96, base),
          };
        });

        const sorted = [...orderedScores].sort(
          (a, b) => b.total - a.total,
        );
        ...
        for (const score of orderedScores) {
          ...
          const position =
            sorted.findIndex(
              (item) => item.student.id === score.student.id,
            ) + 1;
          ...
          await db.orm.public.ResultMark.create({
            resultId: result.id,
            assessmentComponentId: examMark.id,
            mark: examMarkValue,
          });
        }
      }
  ...
  const announcements = [
    [
      "Welcome to the 2025/2026 Session",
      "We welcome all students, parents and staff to a new academic year.",
      "ALL",
    ],
    [
      "First Term Results Published",
      "First term examination results are now available through the portal.",
      "STUDENT",
    ],
    [
      "Fee Payment Reminder",
      "Parents are encouraged to review outstanding balances and keep payments up to date.",
      "PARENT",
    ],
    [
      "Staff Meeting",
      "All teaching staff should review the staff notice before the next academic meeting.",
      "TEACHER",
    ],
    [
      "Third Term Activities",
      "The school has published the major activities planned for the final term.",
      "ALL",
    ],
  ];
  ...
  console.log(
    "Demo password for seeded role accounts:",
    DEMO_PASSWORD,
  );
  console.log(
    "Student portal accounts: student1 - student12",
  );
  console.log("Parent portal account: parent1");
  console.log(
    "Teacher accounts: teacher1 - teacher14",
  );
  console.log("Featured teachers: Musa Ibrahim / Johno Okafor");
